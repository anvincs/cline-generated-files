...
export interface GetUriSchemeResponse {
  uriScheme: string;
}
...
function createBaseGetUriSchemeResponse(): GetUriSchemeResponse {
  return { uriScheme: "" };
}

export const GetUriSchemeResponse: MessageFns<GetUriSchemeResponse> = {
  encode(message: GetUriSchemeResponse, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.uriScheme !== "") {
      writer.uint32(10).string(message.uriScheme);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): GetUriSchemeResponse {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseGetUriSchemeResponse();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
  ...
  fromJSON(object: any): GetUriSchemeResponse {
    return { uriScheme: isSet(object.uriScheme) ? globalThis.String(object.uriScheme) : "" };
  },

  toJSON(message: GetUriSchemeResponse): unknown {
    const obj: any = {};
    if (message.uriScheme !== "") {
  ...
  create<I extends Exact<DeepPartial<GetUriSchemeResponse>, I>>(base?: I): GetUriSchemeResponse {
    return GetUriSchemeResponse.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<GetUriSchemeResponse>, I>>(object: I): GetUriSchemeResponse {
    const message = createBaseGetUriSchemeResponse();
    message.uriScheme = object.uriScheme ?? "";
    return message;
  },
};
...
export const EnvServiceDefinition = {
  ...
  methods: {
    ...
    getUriScheme: {
      ...
      requestType: EmptyRequest,
      requestStream: false,
      responseType: GetUriSchemeResponse,
      responseStream: false,
      options: {},
    },
    /**
...
export const EnvServiceService = {
   ...
   */
  getUriScheme: {
    path: "/host.EnvService/getUriScheme",
    requestStream: false,
    responseStream: false,
    requestSerialize: (value: EmptyRequest) => Buffer.from(EmptyRequest.encode(value).finish()),
    requestDeserialize: (value: Buffer) => EmptyRequest.decode(value),
    responseSerialize: (value: GetUriSchemeResponse) => Buffer.from(GetUriSchemeResponse.encode(value).finish()),
    responseDeserialize: (value: Buffer) => GetUriSchemeResponse.decode(value),
  },
  /**
   * Returns the telemetry settings of the host environment. This may return UNSUPPORTED
   * if the host does not specify telemetry settings for the plugin.
...
export interface EnvServiceServer extends UntypedServiceImplementation {
   ...
   * return an empty uriScheme.
   */
  getUriScheme: handleUnaryCall<EmptyRequest, GetUriSchemeResponse>;
  /**
   * Returns the telemetry settings of the host environment. This may return UNSUPPORTED
   * if the host does not specify telemetry settings for the plugin.
   */
...
export interface EnvServiceClient extends Client {
  ...
  getUriScheme(
    request: EmptyRequest,
    callback: (error: ServiceError | null, response: GetUriSchemeResponse) => void,
  ): ClientUnaryCall;
  getUriScheme(
    request: EmptyRequest,
    metadata: Metadata,
